# WILMS Final Audit Index — v1.4.2

**Date:** 2026-07-21  
**Version:** 1.4.2  
**Verdict:** **READY WITH CONDITIONS** — controlled rollout candidate  
**Production Certified:** **NOT ISSUED**

---

## Phase 31 final certification pack (current)

Primary pack: [`certification/v1.4/phase-31/`](./certification/v1.4/phase-31/)

| Doc | Path |
|-----|------|
| Index | [INDEX.md](./certification/v1.4/phase-31/INDEX.md) |
| Production certification | [FINAL_PRODUCTION_CERTIFICATION_REPORT.md](./certification/v1.4/phase-31/FINAL_PRODUCTION_CERTIFICATION_REPORT.md) |
| Manual actions | [FINAL_MANUAL_ACTIONS_REQUIRED.md](./certification/v1.4/phase-31/FINAL_MANUAL_ACTIONS_REQUIRED.md) |
| Gate manifest | [production-gate-manifest.json](./certification/v1.4/phase-31/production-gate-manifest.json) |

---

## Phase 30 notification pack

[`certification/v1.4/phase-30/`](./certification/v1.4/phase-30/INDEX.md)

---

## Phase 29 certification pack

[`certification/v1.4/phase-29/`](./certification/v1.4/phase-29/INDEX.md)


| Doc | Path |
|-----|------|
| Index | [INDEX.md](./certification/v1.4/phase-29/INDEX.md) |
| Master audit | [FINAL_PHASE_29_AUDIT.md](./certification/v1.4/phase-29/FINAL_PHASE_29_AUDIT.md) |
| Findings (JSON) | [findings-inventory.json](./certification/v1.4/phase-29/findings-inventory.json) |
| Test evidence (JSON) | [test-evidence-manifest.json](./certification/v1.4/phase-29/test-evidence-manifest.json) |
| Production gates (JSON) | [production-gate-manifest.json](./certification/v1.4/phase-29/production-gate-manifest.json) |
| Final certification | [FINAL_PRODUCTION_CERTIFICATION_REPORT.md](./certification/v1.4/phase-29/FINAL_PRODUCTION_CERTIFICATION_REPORT.md) |
| Manual actions | [FINAL_MANUAL_ACTIONS_REQUIRED.md](./certification/v1.4/phase-29/FINAL_MANUAL_ACTIONS_REQUIRED.md) |
| Environment vars | [ENVIRONMENT_VARIABLES.md](./operations/ENVIRONMENT_VARIABLES.md) |

---

## Phase 28 multi-discipline pack (archived)

Primary pack: [`certification/v1.4/phase-28-multi/`](./certification/v1.4/phase-28-multi/)

| Doc | Path |
|-----|------|
| Index | [INDEX.md](./certification/v1.4/phase-28-multi/INDEX.md) |
| Multi-discipline audit | [FINAL_MULTI_DISCIPLINE_AUDIT.md](./certification/v1.4/phase-28-multi/FINAL_MULTI_DISCIPLINE_AUDIT.md) |
| Findings (JSON) | [findings-inventory.json](./certification/v1.4/phase-28-multi/findings-inventory.json) |
| Test evidence (JSON) | [test-evidence-manifest.json](./certification/v1.4/phase-28-multi/test-evidence-manifest.json) |
| Production gates (JSON) | [production-gate-manifest.json](./certification/v1.4/phase-28-multi/production-gate-manifest.json) |
| Final certification | [FINAL_PRODUCTION_CERTIFICATION.md](./certification/v1.4/phase-28-multi/FINAL_PRODUCTION_CERTIFICATION.md) |
| Manual actions | [FINAL_MANUAL_ACTIONS_REQUIRED.md](./certification/v1.4/phase-28-multi/FINAL_MANUAL_ACTIONS_REQUIRED.md) |

---

## Phase 28 initial pack (archived)

| Doc | Path |
|-----|------|
| Index | [INDEX.md](./certification/v1.4/phase-28/INDEX.md) |
| Phase 28 audit summary | [FINAL_PHASE_28_AUDIT.md](./certification/v1.4/phase-28/FINAL_PHASE_28_AUDIT.md) |
| Codebase verification | [PHASE28A_CODEBASE_VERIFICATION.md](./certification/v1.4/phase-28/PHASE28A_CODEBASE_VERIFICATION.md) |
| Defaulter SQL aggregation | [DEFAULTER_SQL_AGGREGATION_REPORT.md](./certification/v1.4/phase-28/DEFAULTER_SQL_AGGREGATION_REPORT.md) |
| Dependency triage | [DEPENDENCY_TRIAGE_REPORT.md](./certification/v1.4/phase-28/DEPENDENCY_TRIAGE_REPORT.md) |
| Migration 0029 verification | [MIGRATION_0029_VERIFICATION_REPORT.md](./certification/v1.4/phase-28/MIGRATION_0029_VERIFICATION_REPORT.md) |
| Staging smoke | [STAGING_SMOKE_REPORT.md](./certification/v1.4/phase-28/STAGING_SMOKE_REPORT.md) |
| Money-chain evidence | [MONEY_CHAIN_EVIDENCE_REPORT.md](./certification/v1.4/phase-28/MONEY_CHAIN_EVIDENCE_REPORT.md) |
| RBAC & security smoke | [RBAC_SECURITY_SMOKE_REPORT.md](./certification/v1.4/phase-28/RBAC_SECURITY_SMOKE_REPORT.md) |
| Backup / restore / DR | [BACKUP_RESTORE_DR_REPORT.md](./certification/v1.4/phase-28/BACKUP_RESTORE_DR_REPORT.md) |
| Load / performance | [LOAD_PERFORMANCE_REPORT.md](./certification/v1.4/phase-28/LOAD_PERFORMANCE_REPORT.md) |
| Production configuration | [PRODUCTION_CONFIGURATION_AUDIT.md](./certification/v1.4/phase-28/PRODUCTION_CONFIGURATION_AUDIT.md) |
| UX / accessibility | [UX_ACCESSIBILITY_FINAL_REPORT.md](./certification/v1.4/phase-28/UX_ACCESSIBILITY_FINAL_REPORT.md) |
| Documentation audit | [DOCUMENTATION_FINAL_AUDIT.md](./certification/v1.4/phase-28/DOCUMENTATION_FINAL_AUDIT.md) |
| Manual actions | [FINAL_MANUAL_ACTIONS_REQUIRED.md](./certification/v1.4/phase-28/FINAL_MANUAL_ACTIONS_REQUIRED.md) |
| Final certification | [FINAL_PRODUCTION_CERTIFICATION.md](./certification/v1.4/phase-28/FINAL_PRODUCTION_CERTIFICATION.md) |
| Release readiness | [FINAL_RELEASE_READINESS.md](./certification/v1.4/phase-28/FINAL_RELEASE_READINESS.md) |

---

## Phase 27 pack (archived)

Primary pack: [`certification/v1.4/phase-27/`](./certification/v1.4/phase-27/)

| Doc | Path |
|-----|------|
| Final Phase 27 audit | [FINAL_PHASE_27_AUDIT.md](./certification/v1.4/phase-27/FINAL_PHASE_27_AUDIT.md) |
| Production certification | [FINAL_PRODUCTION_CERTIFICATION.md](./certification/v1.4/phase-27/FINAL_PRODUCTION_CERTIFICATION.md) |
| Release readiness | [FINAL_RELEASE_READINESS.md](./certification/v1.4/phase-27/FINAL_RELEASE_READINESS.md) |
| Manual actions | [FINAL_MANUAL_ACTIONS_REQUIRED.md](./certification/v1.4/phase-27/FINAL_MANUAL_ACTIONS_REQUIRED.md) |
| Signed invite security | [SIGNED_INVITE_SECURITY_REPORT.md](./certification/v1.4/phase-27/SIGNED_INVITE_SECURITY_REPORT.md) |
| Expense SoD | [EXPENSE_SOD_REPORT.md](./certification/v1.4/phase-27/EXPENSE_SOD_REPORT.md) |
| Financial aggregations | [FINANCIAL_AGGREGATION_AUDIT.md](./certification/v1.4/phase-27/FINANCIAL_AGGREGATION_AUDIT.md) |
| API rate limiting | [API_RATE_LIMITING_REPORT.md](./certification/v1.4/phase-27/API_RATE_LIMITING_REPORT.md) |
| Dependencies | [DEPENDENCY_SECURITY_REPORT.md](./certification/v1.4/phase-27/DEPENDENCY_SECURITY_REPORT.md) |
| Staging smoke | [STAGING_SMOKE_REPORT.md](./certification/v1.4/phase-27/STAGING_SMOKE_REPORT.md) |
| Backup / restore | [BACKUP_RESTORE_DR_REPORT.md](./certification/v1.4/phase-27/BACKUP_RESTORE_DR_REPORT.md) |
| Load / performance | [LOAD_PERFORMANCE_REPORT.md](./certification/v1.4/phase-27/LOAD_PERFORMANCE_REPORT.md) |
| UX / accessibility | [UX_ACCESSIBILITY_REPORT.md](./certification/v1.4/phase-27/UX_ACCESSIBILITY_REPORT.md) |
| Repository hygiene | [REPOSITORY_HYGIENE_REPORT.md](./certification/v1.4/phase-27/REPOSITORY_HYGIENE_REPORT.md) |
| Dead code | [DEAD_CODE_REPORT.md](./certification/v1.4/phase-27/DEAD_CODE_REPORT.md) |
| Security hardening | [SECURITY_HARDENING_REPORT.md](./certification/v1.4/phase-27/SECURITY_HARDENING_REPORT.md) |

Phase 27 closes signed invite tokens, expense SoD, SQL-scoped report aggregations, and global API rate limiting. Operator/staging/DR/load evidence remains **BLOCKED** in this environment — **Production Certified NOT ISSUED**.

---

## Prior packs

| Pack | Path | Verdict |
|------|------|---------|
| Phase 26 | [`certification/v1.4/phase-26/`](./certification/v1.4/phase-26/) | READY WITH CONDITIONS |
| Final system audit (PR #137) | [`certification/v1.4/final-system-audit/`](./certification/v1.4/final-system-audit/) | READY WITH CONDITIONS |
| UX modernisation | [`certification/v1.4/ux-modernisation/`](./certification/v1.4/ux-modernisation/) | Merged software |
| Production operations | [`certification/v1.4/production-operations/`](./certification/v1.4/production-operations/) | Operator runbooks |
