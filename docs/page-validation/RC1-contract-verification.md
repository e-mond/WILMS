# RC1 Contract Verification — Phase 2
...
| PATCH | `/risk-flags/:id/escalate` | — | RiskFlagDetail | REVIEW_RISK_FLAGS |
...
| GET | `/messages/threads` | — | Thread[] | auth |
| POST | `/messages/threads` | collectorUserId, subject? | Thread | auth |
| GET | `/messages/threads/:id` | — | ThreadDetail | auth |
| POST | `/messages/threads/:id/messages` | body | Message | auth |
| PATCH | `/loans/:id/approve` | — | LoanDetail | APPROVE_LOANS |
...
| GET | `/sync/conflicts` | — | Conflict[] | sync perm |
| POST | `/sync/conflicts/:id/approve` | — | result | sync perm |
...
**PASS** — Frontend services match backend routes; Zod validation on all new mutation routes.
