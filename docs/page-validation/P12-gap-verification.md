...
Re-verified against the codebase on 2026-06-09. Baseline: `P11j-incomplete-work-audit.md`. P11a–P11j UI work was not reopened.
...
| 5 | Leader permissions — service enforcement | Partial | **Complete (demo)** | Frontend | `assertCanManageGroupLeader()` in `groupService.mock.ts` uses `canManageGroupLeader()` + actor role from `DEMO_ACCOUNTS` / settings users store. Tests in `groupService.mock.display-name.test.ts`. |
| 6 | Automated group formation — production path | Partial | **Partial** | Backend | Mock path intact (`groupFormationService.mock.ts` → `group-formation.store.ts`). API stub in `groupFormationService.ts`; no `/groups/formation/*` routes. |
| 7 | Settings editable beyond min/max | Partial | **Partial** | Backend | `SettingsPanel.tsx` — only min/max group size editable via `updateSettings`; other rows remain `readOnly` / `disabled`. |
