...
```
KpiCard value → hook/query → @/services → mock factory / API provider
```
...
**Non-KPI (acceptable):** `QUICK_ACTIONS` constant — navigation links only, not metrics.
...
|---|---|---|---|
| Hero amount / target / % / paid / pending / overdue | `CollectorDashboardPanel` | `hero`, `summary` | `collectorService.getDashboard()` → `assembleCollectorDashboard()` |
| Groups due / streak / trend / payments / rate | `CollectorDashboardPanel` | `stats`, `todayGroups` | Same |

**Status:** Pass — refactored in P11c; no hardcoded streak/trend values.
...
**Auditor portal:** Uses same report panels under `/auditor/reports` — same service paths.
...
|---|---|---|
| `CollectorProfilePanel` sidebar lists / group table | Inline demo copy | P1 — not KPI cards; detail panel enhancement |
