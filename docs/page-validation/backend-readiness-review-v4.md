...
**Yes — backend integration can begin immediately** for read-heavy domains and CRUD surfaces that already consume `@/services`. The frontend architecture is mock-first with a clean provider switch (`MockDataProvider` ↔ `ApiDataProvider`). No UI rewrites are required to swap data sources.
...
| Provider wiring | Ready | `index.development.ts` → mock; `index.production.ts` → API or mock fallback |
| Demo fallback | Ready | `NEXT_PUBLIC_DEMO_MODE=true` keeps UI populated |

### Domain services — ready for API wiring
...
## Collector dashboard — backend readiness

The collector dashboard is **fully service-driven** after P11c:

```
UI → collectorService.getDashboard()
       → assembleCollectorDashboard() [mock]
       → buildCollectorDashboardCore() + metrics + groups factory
```
...
1. **Auth + session** — login, session cookie, embed permission snapshot
2. **Read-only domains** — borrowers list, groups, collectors, reports, dashboard KPIs
3. **Write paths** — registration submit, approver decisions, payment record, reconciliation
4. **Uploads** — registration photos, ID scans, signatures
5. **Audit + notifications** — append-only log, optional real-time channel
