...
## 5. Group leader permissions — service enforcement
...
## 6. Automated group formation — production path

| Field | Value |
|-------|-------|
| Feature | Auto-create groups at approval threshold |
| Status | **Partial** |
| Evidence | Fully wired in mock: `borrowerService.mock.ts` → `groupFormationService.mock.ts` → `group-formation.store.ts`. API: `groupFormationService.ts` posts to `/groups/formation/*` (no route). |
...
## 7. Settings — editable configuration

| Field | Value |
|-------|-------|
| Feature | Super Admin system settings |
| Status | **Partial** |
| Evidence | Only `minGroupSize` / `maxGroupSize` editable via `SettingsPanel` → `updateSettings`. Security, SMS, notifications, loan caps, integrations remain `readOnly` / `disabled`. |
