...
| Remove stale `WILMS_GIT_COMMIT` | Ops — pending | Railway dashboard |
| Railway/Vercel same commit live | Ops — pending | Post-deploy curl |

### Live smoke evidence (2026-07-03)

```
npm run smoke:production  →  Passed: 31/31
/health → gitCommit=4e0b10a, schema.status=ok, migrations 11/11
```
...
groups, pools, risk-flags, messages, reports, portfolio — all 200, brotli JSON ok).
...
## Phase 11 — Super Admin verification
