...
**Result: PASS (critical)** / **PARTIAL (overall)** — No critical npm vulnerabilities; production auth/RBAC controls functional; list-endpoint 500s are availability not auth bypass.
...
→ 0 critical
→ 9 high, 9 moderate (unchanged from RC1.2)
```

**Action:** Document only — do **not** run `npm audit fix --force`.
...
| Gate | Result |
|------|--------|
| smoke:rbac | **7/11** — failures are **500**, not 403 bypass |
...
| TD-04 | Stateless session — no server revocation | Medium |
| TD-05 | Demo smoke credentials default | High — rotate to `@wilms.production` |
