...
- Photos: `uploadService` → `POST /uploads` (CSRF + `CAPTURE_DOCUMENTS`)

## RBAC

`assertCollectorAccess()` — collector may only access own `collectorId`; admin override.

## Verdict

**PASS** — Collector portal live; smoke + integration tests green.
