...
|--------|---------------|----------------------|---------------|-----|
| **Create** (register borrower) | `REGISTER_BORROWERS` | Yes — officer portal | No `PermissionGate` on wizard submit | Route-only |
| **Edit** (borrower / pending) | `EDIT_BORROWERS`, `EDIT_PENDING_REGISTRATIONS` | Partial — portal routes | No action gates | Route-only |
...
| **Reset PIN** | `RESET_PIN` | Settings | **Gated** in profile modal | — |
| **Reset Password** | `RESET_PASSWORD` | Settings | **Gated** in profile modal | — |
...
| **Edit display name** | `MANAGE_GROUPS` | `/groups` | **Gated** — `GroupDisplayNameSection.tsx` | — |
...
| All other create/edit/approve/export/print/assign flows | **Route access only** — no `PermissionGate` on individual buttons |

---

## Recommendations (no changes made — audit only)

Roll out existing `PermissionGate` to high-risk actions if fine-grained RBAC is required beyond portal boundaries:

1. `WilmsExportActions` — wrap with `EXPORT_REPORTS` where reports are auditor-scoped
2. Approval actions — `APPROVE_BORROWERS` / `REJECT_LOANS`
3. Group mutations — `MANAGE_GROUPS` on flag/reassign/membership panels
4. User edit modal suspend/delete — extend gates already used in profile modal
