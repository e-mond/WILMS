...
**Result:** **FAIL** — production list endpoints returning HTTP 500
...
```
✗ bff-proxy-dashboard: status=500
✗ bff-proxy-groups: status=500
✗ bff-proxy-loan-pools: status=500
✗ bff-proxy-risk-flags: status=500
✗ bff-proxy-messages: status=500
✗ bff-proxy-collectors: status=500
✗ bff-proxy-borrowers: status=500
✗ bff-proxy-loans-portfolio: status=500
```

**Likely cause:** Production database rebuilt (tables regenerated) — backend list aggregations may error on empty/partial schema state or missing reference joins. Requires Railway log review and post-rebuild API fix deploy.
