...
| Check | Status | Notes |
|-------|--------|-------|
| Brand left only | Pass | `OfficeShellMobileBar` — logo + truncated title |
| Notifications + profile right | Pass | `ShellNavbarActions` with `mobileSimplified` |
| Overflow menu (`MoreVertical`) | Pass | `ShellMobileOverflowMenu` — Search, Settings, App Lock, Theme, Connection |
...
| Check | Status | Notes |
|-------|--------|-------|
| Fixed FAB at left edge | Pass | `MobileSidebarTrigger` — `fixed left-0 top-1/2 -translate-y-1/2 z-[60]` |
...
| Check | Status | Notes |
|-------|--------|-------|
| Mobile keypad-only | Pass | `useCoarsePointerDevice()` → `inputMode="none"` + `readOnly` |
...
| Demo mock coordinates | Pass | `locationService.mock` → `DEMO_CURRENT_LOCATION` |
| Permission / unavailable messages | Pass | Friendly errors surfaced via `role="status"` |

## 6. Region → District → City
...
| Check | Status | Notes |
|-------|--------|-------|
| 2-page layout | Pass | `registration-agreement-print-html.ts` — page 1 + page 2 |
