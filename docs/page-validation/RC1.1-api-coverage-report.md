...
|--------|-------|---------------|---------|
| Super Admin | dashboard, borrowers, loans, groups, pools, collectors, risk, reports, settings, adjustments | `*Service.ts` → `apiClient` | Express modules |
...
- Production build aliases `@/services/index` → `index.production.ts`
...
Detail/read/validation endpoints consumed via dynamic paths — documented in `RC1.1-api-matrix.md`.

## Verdict

**PASS** — Every static `apiClient` path has a backend route; no placeholder UI; no mock leakage in features.
