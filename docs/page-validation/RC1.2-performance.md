...
**Result:** PASS (budgets) / **PARTIAL** (Lighthouse performance −1 pt)

## Bundle budgets

```
JS total (gzip): 168.5 KB (budget 350 KB) — PASS
CSS total (gzip): 8.2 KB (budget 100 KB) — PASS
...
## Lighthouse — production login (unauthenticated)
...
Likely contributors: Vercel edge cold start, first-byte latency to Neon-backed BFF health, font/CSS pipeline on login. **Does not block v1.0.0** — documented non-blocking gap (TD-08 class).
