...
| Repository → DataSource pattern | Ready | `IDataProvider` / `IDataSource` switch |
...
1. **API base URL + endpoint implementation** — backend team
2. **Session permission snapshot** — for override-aware middleware
3. **Upload service** — photos, documents, QR capture completion
4. **Append-only audit API** — compliance requirement
5. **Real-time channel** — notifications and dashboard (P1)
...
All mock data centralized in `mocks/` + `services/mock/` — no inline arrays in components.
