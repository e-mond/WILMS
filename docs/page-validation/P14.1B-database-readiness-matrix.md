# P14.1B — Database Readiness Matrix
...
Documentation-only phase — no migrations, schema, or code changes.
...
| Domains at ≤35% overall | 8 — Users, Collectors, Expenses, Reconciliation, Notifications, Search, Settings, Dashboard |
...
|-----------|-------|----------|
| Frontend | 95% | Login, session, guards — `authentication/`, `lib/auth/*` |
| Backend | 45% | Login + session GET only — `modules/auth` |
...
| Backend | 70% | CRUD, checks, approval — `modules/borrowers` |
...
- [x] Entity relationships documented — `P14.1B-entity-relationship-discovery.md`
- [x] Table blueprint from types — `P14.1B-database-blueprint.md`
- [x] Contract gaps identified — `P14.1B-dto-contract-audit.md`
- [x] Upload model analyzed — `P14.1B-upload-storage-analysis.md`
- [x] Domain readiness scored — this document
- [ ] Product sign-off on enum alignment breaking changes
- [ ] Decision: Collector as User role vs separate table (types imply user id overlap — `collector-management.ts`, `mocks/users.ts`)
