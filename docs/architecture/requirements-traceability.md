...
| REQ-034 | §9.3 | Same-day corrections permitted for Collectors only | Payment Collection | `/collector/payment/[id]` | Payment reverse + re-record (UI) | authStore (`useAuth`) | `PATCH /payments/[id]` → **409 immutable**; corrections via reversal/adjustment | Domain: posted payments immutable (Phase 33 H5); mock path may still edit | ⚠️ Superseded by immutable ledger |
| REQ-035 | §9.3 | All edits logged (timestamp, user, reason) | Payment Collection | Audit Log | — | — | Reversal/adjustment audit actions | Corrections logged via reversal/adjustment workflows | ✅ Complete |
