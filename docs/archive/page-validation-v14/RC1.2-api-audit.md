...
RC1 API Coverage — Pages: 46 — Placeholder hits: 0
...
| Module | Pages | Service → Route | Status |
|--------|-------|-----------------|--------|
| Super Admin | 14 routes | `*Service.ts` → `apiClient` → Express modules | PASS |
...
Detail/read/validation endpoints (e.g. `GET /borrowers/:id`, `GET /reconciliations/:id`) — **intentional**, consumed via dynamic paths. Not frontend gaps.

## Spot-check mutation flows (production API)

| Flow | Route | Shape | Status |
|------|-------|-------|--------|
| Approve borrower | `POST /borrowers/:id/approve` | `{ decision, comment? }` → envelope + audit | Verified via smoke + unit tests |
| Record payment | `POST /payments` | allocation payload → payment record | Verified via financial harness |
| Create group | `POST /groups` | group metadata → `systemId` | Verified via API integrity |
| Upload | `POST /uploads` | multipart → Cloudinary URL | `cert:upload:smoke` PASS (RC1.1) |
| Raise risk flag | `POST /risk-flags` | entity + severity → flag id | Verified via module tests |
