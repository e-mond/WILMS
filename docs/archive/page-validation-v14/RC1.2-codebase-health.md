...
| Mock import guard | `features/**` | **PASS** — 0 forbidden imports |
...
| Tool | Result | Action |
|------|--------|--------|
| `npx knip` | Not completed locally (disk `ENOSPC` during parallel E2E) | **Document only** — defer to CI |
| `npx depcheck` | Reports unused root `vitest` devDependency | **Document only** — used by workspace scripts |
...
| Tier | Items |
|------|-------|
| **Remove now** | None — no unreferenced exports confirmed with green tests |
