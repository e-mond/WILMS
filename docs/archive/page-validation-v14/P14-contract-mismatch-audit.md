...
| `simulatePhoneCapture` | API stub rejects (`photoCaptureSessionService.ts:13-14`) | Not implemented — demo-only per contract map |
...
| 15+ service domains | Stubs exist, no backend module yet | Loans, settings, notifications, search, collectors, risk flags, etc. — P14.1+ |
...
| Upload | `IUploadService` | `upload.store.ts` | `/uploads` | ✅ |
| Borrowers | `IBorrowerService` | `borrowerService.mock.ts` | `/borrowers/*` | ✅ partial |
| Group formation | `IGroupFormationService` | `groupFormationService.mock.ts` | `/groups/formation/*` | ✅ |
| Payments | `IPaymentService` | `paymentService.mock.ts` | `/payments` | ✅ partial |
| Audit | `IAuditService` | `auditService.mock.ts` | `/audit`, `/audit-log` | ✅ |
| Reports | `IReportService` | `reportService.mock.ts` | `/reports/*` | ✅ hub + daily collection |
