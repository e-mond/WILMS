...
**Date:** 2026-06-09  
**Scope:** Evidence from `src/services/mock/`, stores, sync modules, and API stub rejections. Frontend remains source of truth; no P11–P13 regressions reopened.

## 1. Explicit API stub rejections (cannot fulfill without backend)

| Method | File:line | Mock behavior |
|--------|-----------|---------------|
| `deleteRegistration` | Was `borrowerService.ts:83-86` | Mock: `borrowerService.mock.ts:393+` — **P14 backend implements DELETE** |
| `simulatePhoneCapture` | `photoCaptureSessionService.ts:13-14` | Mock: `photoCaptureSessionService.mock.ts:41-58` — **still mock-only** |
...
|------|----------|----------------|
| Auto group formation on approve | `borrowerService.mock.ts:346-351` → `groupFormationService.mock.ts` | ✅ `processApprovedBorrower` in backend |
| Registration approved/rejected notifications | `borrowerService.mock.ts:337-368` | ❌ Not emitted server-side |
| Payment duplicate detection | `paymentService.mock.ts:48-55` | ✅ `findDuplicatePayment` + 409 |
| Payment → loan schedule application | `paymentService.mock.ts: applyPaymentToOldestObligation` | ❌ Not in P14 payments module |
| Overpayment review queue | `paymentService.mock.ts` + `overpaymentReviewService.mock.ts` | ❌ |
| Loan disbursed/completed notifications | `loan-notifications.sync.ts` | ❌ |
| Payment reminder cron | `payment-reminder.sync.ts:57-64` | ❌ |
| Borrower escalation | `borrower-escalation.sync.ts` | ❌ |
| Adjustment audit trail | `adjustmentService.mock.ts:126+` | ❌ |
| Synthetic borrower profiles by ID pattern | `borrower-full-profile.builder.ts` | ❌ Backend 404 only |

## 4. Hardcoded IDs & demo data

| Item | Evidence |
|------|----------|
| Demo users | `constants/demo-accounts.ts` — IDs `user-super-admin`, `user-collector`, etc. |
| Backend seed | `backend/src/seed/demo-users.ts` mirrors same IDs/passwords |
| Pre-seeded pending borrowers | `backend/src/db/store.ts` — `borrower-pending-001`, `borrower-pending-002` |
| Mock registry entries | `mocks/borrower-registry.ts` — `borrower-001`, etc. (demo mode only) |
...
Stubs call `apiClient` paths documented in `P13-api-contract-map.md` — no server handlers until subsequent P14 phases.
...
| Webpack services alias | `next.config.mjs:63-85` | Dev → mock; prod → `index.production.ts` |
