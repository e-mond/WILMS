# Hardcoded Data Audit — v3 (P11b)

> Repository scan focus: UI components, pages, layout | Date: 2026-06-09

## Compliant patterns

- Dashboard KPIs, collection metrics, expense summaries → services/selectors
- Registration legal copy → `settingsService.getRegistrationLegalConfig()`
- Guarantor limits → `checkGuarantorEligibility` service
- Role labels → `getRoleLabel()` from session
...
No new raw hex values introduced in P11b shell/registration pass. Signature pad stroke uses `#111827` (maps to design token — consider `text-text-primary` via canvas refactor).
