# P14.1C — Relationship Architecture
...
|------|-----|-------------|-----------|-------------------|-------------------|--------|
| users | collectors | 1 : 0..1 | User owns collector profile | RESTRICT — cannot delete user with active collector assignments without workflow | users.deleted_at hides collector ops | **Confirmed** ADR-001 |
...
| users | audit_entries | 1 : * (as actor) | Platform | RESTRICT — preserve audit | Audit never deleted | **Confirmed** |
...
| collectors | groups | 1 : * (assignment) | Platform assigns | SET NULL group.collector on collector soft-delete | Groups need reassignment | **Referenced** — `ReassignGroupCollectorInput` |
...
| groups | borrowers | 1 : * (via members) | Group | — | — | **Confirmed** |
| groups | users (leader) | * : 1 | Leader is borrower | RESTRICT | — | **Referenced** `GroupLeaderProfile` |

**Ambiguity (P14.1B):** `GroupRecord.memberIds[]` vs normalized `group_members` — **Resolved for P14.1C:** use join table; deprecate array on record.
...
| borrowers | uploads | * : * (via FK columns) | Borrower references | RESTRICT — uploads retained for audit | Orphan uploads garbage-collected **Proposed** | **Referenced** gap on backend |
...
**Ambiguity (P14.1B):** Registration as separate entity — **Resolved:** workflow on Borrower aggregate, not separate table.
...
| From | To | Cardinality | Ownership | Cascade | Soft delete impact | Status |
|------|-----|-------------|-----------|---------|-------------------|--------|
| borrowers | approval state | 1 : 1 (status column) | Borrower aggregate | N/A | — | **Confirmed** |
| approval action | group_formation | 1 : 0..1 trigger | Formation service | N/A | — | **Confirmed** backend |
| approval action | notifications | 1 : * side effect | Notification domain | N/A | — | **Referenced** mock only |

**Ambiguity (P14.1B):** Reviewed applications list — **Resolved:** query `borrower_approval_decisions` or materialized view; not separate aggregate root.
...
| From | To | Cardinality | Ownership | Cascade | Soft delete impact | Status |
|------|-----|-------------|-----------|---------|-------------------|--------|
| loans | loan_schedule_weeks | 1 : * | Loan | CASCADE | — | **Confirmed** |
| loans | payments | 1 : * (allocation) | Payment | RESTRICT | — | **Referenced** — not on PaymentRecord today |
| loans | financial_transactions | 1 : * | Ledger | RESTRICT | — | **Referenced** mock |
| borrowers | financial_transactions | 1 : * (admin fee) | Transaction | RESTRICT | — | **Confirmed** types |
| payments | users (collector) | * : 1 | Collector records | RESTRICT | Maps to users.id not collectors.id | **Confirmed** ADR-001 |
| payments | audit_entries | 1 : * | Audit | RESTRICT | — | **Confirmed** partial backend |

**Ambiguity (P14.1B):** `collectorId` on payment — **Resolved (ADR-001):** FK to `users.id`; join `collectors` for operational fields in queries.
...
| expenses | uploads | * : 0..1 (receipt) | Expense | SET NULL | — | **Referenced** |
| users | reconciliation_submissions | 1 : * | Collector | RESTRICT | — | **Referenced** mock |
| payments | reconciliation (derived) | * : 1 aggregate | Computed | N/A | — | **Referenced** |
...
| notifications | borrowers | * : 0..1 | Context | SET NULL | — | **Confirmed** optional FK |
| notifications | loans | * : 0..1 | Context | SET NULL | — | **Confirmed** |
| domain mutations | notification_deliveries | 1 : * | Platform | RESTRICT | — | **Referenced** mock producers |

**Ambiguity (P14.1B):** Inbox without userId on DTO — **Resolved:** `notifications_inbox.user_id` NOT NULL in schema.
...
| uploads | borrowers | * : 0..1 (entity) | Polymorphic optional | RESTRICT binary | — | **Confirmed** entityId |
| uploads | expenses | * : 0..1 | Receipt | RESTRICT | — | **Referenced** |

**Ambiguity (P14.1B):** Base64 in DB — **Resolved (phase brief):** external storage_key only; no binary in PostgreSQL.
...
| risk_flags | risk_flag_timeline_events | 1 : * | Flag | CASCADE | — | **Confirmed** mock |
...
| notifications | deleted_at | Hide from inbox | — |
| expenses | deleted_at | Hide from claims | — |
...
## Ambiguous relationships — resolution log
...
| 6 | Loan pool FK | loan_pool_id nullable on loans | No FK in types — optional |
