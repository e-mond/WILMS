# P14.1C — Migration Architecture Plan

**Phase:** P14.1C (planning only — no migrations, no SQL)  
**Date:** 2026-06-09  
...
- Neon project + database provisioning (**Proposed** — not created in P14.1C)
...
- ADR-002 file referenced in brief not in repo — rules applied from phase brief
...
- Canonical UserStatus enum merge (frontend `ACTIVE|INVITED|SUSPENDED` — `settings.ts`)
...
- External storage wiring (replace base64 — phase brief)
- Multipart API contract (P14.1B gap)

### Risks

- Registration flow depends on upload IDs before borrowers table exists — uploads can land with entity_id null initially
...
## Phase 3 — Borrowers + Registration + Approvals
...
BorrowerStatus, ApprovalStatus (maps ReviewedDecision — `approval.ts`)

### Blockers

- Enum merge: frontend 6 values vs backend 4 — **Breaking change risk** (P14.1B #7)
- All registration upload FK columns
- Soft delete vs hard delete registration — policy decision
...
- Group management API still mock — schema ahead of API **acceptable**
...
## Phase 5 — Payments + Expenses + Reconciliation
...
- PaymentStatus canonical enum — **Breaking** (P14.1B #1)
...
- Entire loan API module absent — largest backend gap
...
Phase 1 users; mutation hooks from Phases 3–6
...
| Level | Action |
|-------|--------|
| L1 — Feature flag | `DATA_PROVIDER=memory` bypasses DB repositories |
| L2 — Phase rollback | Drop tables from failed phase forward (dev branches only) |
| L3 — Neon branch | Reset to pre-migration branch snapshot |
| L4 — Application | Redeploy previous release; in-memory backend still exists in P14.0 code |

**Evidence:** Current dual provider — `MockDataProvider` vs `ApiDataProvider` — `src/data-provider/types.ts`
...
2. Neon not provisioned (0% deployed — P14.1B matrix)
...
| R3 | Schema ahead of API | 4–8 | Feature flags per domain |
