...
Production build uses `index.production.ts` → `ApiDataProvider` (webpack alias).
...
- `utils/collector-management-list.ts` (partial — uses DEMO_ACCOUNTS for mock builders)
...
**PASS** — No user-facing production code depends on mock data handlers.
