...
### 1. Loans / Disbursements — `/loans`

**Component:** `src/features/loan-management/components/LoanPortfolioList.tsx`

| Control | Implementation | Status |
|---------|----------------|--------|
| Search | Controlled `Input`, `searchQuery` → `filterPortfolioEntries()` | Functional |
...
**Tests:** `src/tests/loan-management/LoanPortfolioList.test.tsx` — portfolio render + status filter (`Completed` pill) pass.

**Result:** PASS — no export/action regression found.

---

### 2. Risk & Flags — `/risk-flags`
...
| Action | `Raise Flag` button → `RaiseFlagModal` | Present |
...
**Result:** PASS — export and Raise Flag remain in `actions` slot; no toolbar structure change in P11j.

---

### 3. Auditor Reports — `/auditor/reports`
...
**Result:** PASS — report-specific toolbars retain their own `actions` wiring; shared component only affects layout classes.

---

## Responsive notes (code-level)

- **Mobile:** Filter pills use horizontal scroll (`overflow-x-auto`); actions stack in column below filters — intentional.
- **Tablet (md–lg):** Actions may wrap (`sm:flex-wrap`) until `lg:flex-nowrap` — pills scroll rather than clip.
