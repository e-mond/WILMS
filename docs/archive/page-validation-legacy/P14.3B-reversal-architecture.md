# P14.3B ÔÇö Reversal Architecture

**Phase:** P14.3B.3B  
**Date:** 2026-06-21  
**Status:** Design complete ÔÇö pending human approval before 3C implementation
...
Reversal execution (atomic)
        Ôö£ÔöÇ reversal record (APPROVED/EXECUTED)
        Ôö£ÔöÇ compensating ledger entry (REVERSAL)
        Ôö£ÔöÇ compensating pool allocation (if applicable)
        Ôö£ÔöÇ loan balance + schedule correction
        Ôö£ÔöÇ source entity status flag (REVERSED)
        ÔööÔöÇ reversal_history + audit
```
...
- Schedule week linked to payment can revert from PAID ÔåÆ prior status (MISSED or PENDING per due date)

**Transaction sequence**

```text
BEGIN
  1. INSERT financial_reversals (PENDING ÔåÆ EXECUTED in same txn for admin-execute model)
  2. UPDATE payments SET status=REVERSED, reversed_by, reversed_at, version+1
     WHERE id=? AND version=? AND status=CONFIRMED
  3. UPDATE loan_schedules week: PAID ÔåÆ computed status (optimistic version)
...
- WRITE_OFF reversal triggers dedicated workflow (blacklist restore out of scope Phase 3C ÔÇö **Blocking Issue B-ARCH-02**)
...
Not a standalone user action ÔÇö **sub-step** of payment or adjustment reversal.
...
- Within policy window (configurable ÔÇö design default: same cycle batch only)

**If any payment exists:** **Reject** ÔÇö use payment reversal path instead.

**Transaction sequence:** Reverse lifecycle toward PENDING_DISBURSEMENT; compensating `LOAN_DISBURSEMENT` offset via `REVERSAL` ledger entry; soft-flag disbursement row `reversed_at` (new column design ÔÇö no delete).

**Phase 3C scope:** **Deferred** unless preconditions trivial ÔÇö document as Phase 3D candidate.
...
Add enum value: **`REVERSAL`** (design only ÔÇö migration in 3C).
...
| Column | Type | Purpose |
|--------|------|---------|
| `reverses_ledger_entry_id` | uuid nullable FK ÔåÆ `ledger_entries.id` | Direct link to reversed entry |
| `reversal_id` | uuid nullable FK ÔåÆ `financial_reversals.id` | Link to reversal workflow row |

Until FK exists, require `metadata.reversesLedgerEntryId` (interim ÔÇö not preferred for production).

### Balance recalculation strategy

**Authoritative:** `loans.loan_balance` updated atomically in reversal txn (same as payment post).

**Verification:** `verify:reversals` harness recomputes expected balance from disbursements ÔêÆ repayments ┬▒ adjustments ┬▒ reversals.

**Do not** recalculate from scratch on every read ÔÇö preserve current denormalized pattern.
...
## API Design (3C ÔÇö not implemented)
...
## Unresolved ÔåÆ Blocking Issues

| ID | Issue | Disposition |
|----|-------|-------------|
| B-ARCH-01 | Adjustment vs reversal mutual exclusion on same payment | Resolved in design ÔÇö enforce at API |
| B-ARCH-02 | WRITE_OFF adjustment reversal (blacklist restore) | **Deferred to Phase 5** ÔÇö block reversal of WRITE_OFF in 3C |
| B-ARCH-03 | Schedule week status after unpay | Design: revert to MISSED if dueDate < today else PENDING |
| B-ARCH-04 | Disbursement reversal scope | **Deferred to 3D** ÔÇö payment reversal only in 3C MVP |

**Phase 2 exit:** PASS ÔÇö no unresolved questions block payment reversal architecture.
