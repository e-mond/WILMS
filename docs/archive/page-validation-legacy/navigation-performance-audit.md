...
| 12├ù `usePathname` + `useSearchParams` per nav | Re-render storm on every route change | `ShellNavLink` ├ù 12 |
...
| Mock 300ms delay ├ù multiple queries | Risk/Settings/Reports feel sluggish | `simulateDelay()` |
...
1. **Split `AsideSlotContext`** into stable dispatch + content contexts ÔÇö subscribers no longer re-render when only unrelated state changes.
...
6. **`(super-admin)/loading.tsx`** ÔÇö instant loading spinner during transitions.
7. **`next/dynamic`** on heavy pages: Settings, Loan Pools, Risk & Flags, Reports.

### Bundle weight

8. **`WilmsExportActions`** ÔÇö Excel/PDF engines dynamically imported on button click; CSV/print remain synchronous.

### Page fixes

9. **`LoanPoolsPanel`** ÔÇö stable activity timestamps; dedicated **`LoanPoolsMobileCardList`** for mobile (table hidden `<lg`).
10. **`CollectorDashboardPanel`** ÔÇö mobile borrower cards + quick actions row; desktop table preserved `md+`.
...
- **No aside nullÔåÆcontent flicker** from unstable context.
...
npm run type-check
npm run dev
# Click: Loan Pools ÔåÆ Risk & Flags ÔåÆ Reports ÔåÆ Settings
```
