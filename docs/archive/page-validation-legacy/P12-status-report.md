...
Report date: 2026-06-09. All claims verified against current codebase. P11aÔÇôP11j work was not reopened.
...
| **Formation queue visibility** | `GroupFormationStatusSection.tsx` ÔåÆ `groupFormationService.getCommunityStatus()` |
...
| Area | Gap |
|------|-----|
| Production REST API | All non-auth service endpoints ÔÇö stubs only |
...
| E2E | Group assignment, expenses, reports, user management, search ÔÇö no specs; `role-journeys` mobile drawer may be stale |
...
|-----------|--------|
| Interface | Complete |
| Mock | Complete ÔÇö all 5 `UPLOAD_PURPOSE` values |
| API stub | Complete ÔÇö `POST/GET/DELETE` paths |
| Provider registration | Complete |
| Unit tests | Complete ÔÇö `uploadService.mock.test.ts` |
...
| `PermissionGate` on actions | **Partial** ÔÇö settings admin actions + group display name only |
| Service-layer auth (mock) | Partial ÔÇö group leader replace enforced; other services unchecked |
...
| **Weighted backend readiness** | **~35%** ÔÇö `(100% contracts + 4% routes) / 2`, rounded |
...
| `npm run lint` | Pass |
| `npm run test` | Pass ÔÇö **399 tests** (75 files ├ù 2 shards) |
| `npm run build` | Pass ÔÇö 42 routes |
