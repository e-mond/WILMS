...
function toDetail(record: BorrowerRecord, sequence?: number) {
  return {
    ...
    businessPremisesNumber: record.profile.businessPremisesNumber,
    typeOfWork: record.profile.typeOfWork,
    typeOfWorkOther: record.profile.typeOfWorkOther,
  ...
  const record: BorrowerRecord = {
    ...
    profile: {
      ...
      businessPremisesNumber: payload.businessPremisesNumber
        ? String(payload.businessPremisesNumber)
        : undefined,
      businessAddress: String(payload.businessAddress ?? ''),
      typeOfWork: String(payload.typeOfWork ?? ''),
      typeOfWorkOther: payload.typeOfWorkOther ? String(payload.typeOfWorkOther) : undefined,
      guarantorName: String(payload.guarantorName ?? ''),
      guarantorPhone: String(payload.guarantorPhone ?? ''),
