...
export async function autocompleteLocations(query: string, limit = 12) {
  const normalized = query.trim();
  if (!normalized) {
    return {
  ...
  const rows = await locationMasterRepo.searchLocationsRanked(normalized, limit);
  const sync = await locationMasterRepo.getLatestLocationSync();
  return {
    meta: {
      version: sync?.datasetVersion ?? 'unknown',
