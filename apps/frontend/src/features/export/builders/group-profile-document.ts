...
export function buildGroupProfileExportDocument(
  ...
  const groupDisplayId = resolveGroupDisplayId({
    id: group.id,
    groupSystemId: group.groupSystemId,
  ...
  return {
    metadata: {
      ...
      generatedBy,
      referencePrefix: 'GRP',
      entityRef: groupDisplayId,
    },
    executiveSummary: `${group.name} (${groupDisplayId}) is ${group.statusLabel.toLowerCase()} in ${group.community}. The group has ${group.activeMemberCount} active members, ${group.activeLoanCount} active loans, and a collection rate of ${group.collectionRatePercent}%. Outstanding balance is ${formatPesewasForExport(group.outstandingPesewas)}.`,
    sections: [
      {
        title: 'Group Summary',
        type: 'summary',
        summaryItems: [
          { label: 'Group ID', value: groupDisplayId },
      ...
      },
      {
        ...
        summaryItems: [
          ...
          { label: 'Borrower ID', value: resolveBorrowerDisplayId({ id: group.leader.borrowerId }) },
      ...
      },
      {
        ...
        summaryItems: [
          ...
          { label: 'Collector ID', value: resolveCollectorDisplayId(group.collector) },
      ...
      },
      {
        ...
        table: {
          ...
          rows: group.members.map((member) => [
            resolveBorrowerDisplayId({ id: member.borrowerId }),
            member.fullName,
      ...
      },
      {
        ...
        summaryItems: [
          ...
          { label: 'Entity Reference', value: groupDisplayId },
        ],
      },
    ],
    signatures: [
