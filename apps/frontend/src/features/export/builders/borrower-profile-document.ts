...
export function buildBorrowerProfileExportDocument(input: BorrowerProfileExportInput): WilmsExportDocument {
  ...
  const locationRows = buildLocationHierarchyRows({
  ...
  if (variant === 'full' || variant === 'loan-summary') {
    sections.push({
      ...
      summaryItems: [
        ...
        { label: 'Borrower ID', value: borrowerDisplayId },
        ...
        ...locationRows.map(([label, value]) => ({ label, value })),
  ...
  return {
    metadata: {
      ...
      referencePrefix: 'BRW',
      entityRef: borrowerDisplayId,
    },
    executiveSummary: `${borrower.fullName} (${borrowerDisplayId}) borrower report generated for official WILMS record keeping.`,
    sections,
